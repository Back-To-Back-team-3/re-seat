    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="주요 메뉴">
          ...
          <button className={activeStep === "tickets" ? "active" : ""} onClick={openTickets} disabled={!isAuthed}>내 티켓</button>
        </nav>
      ...
      {isAuthed && !isVerified ? (
        <main className="verification-page">
          <Alert error={error} toast={toast} onClose={() => { setError(null); setToast(null); }} />
          <section className="verification-card">
            <div className="verification-visual">
              ...
              <ol className="verification-steps">
                <li className="done"><span>1</span><div><strong>카카오 로그인</strong><small>계정 확인 완료</small></div></li>
                <li className="active"><span>2</span><div><strong>휴대폰 본인인증</strong><small>현재 단계</small></div></li>
                <li><span>3</span><div><strong>경기 예매 시작</strong><small>인증 후 바로 이용</small></div></li>
              </ol>
            </div>
            <div className="verification-action">
              ...
              <div className="verification-note"><span>⌕</span><div><strong>인증 정보는 안전하게 처리됩니다.</strong><small>인증 결과는 중복 예매 방지와 회원 식별에만 사용됩니다.</small></div></div>
              <button className="primary-button full" disabled={busy} onClick={() => startIdentityVerification()}>
        ...
        <>
          {activeStep !== "games" && activeStep !== "tickets" && (
            <div className="flow-header">
              <button onClick={goHome}>홈</button><span>›</span>
      ...
      <footer className="footer">
        ...
        <p>© 2026 Re:Seat. All rights reserved. · <a href="https://commons.wikimedia.org/wiki/File:Doosan_Bears_vs_LG_Twins_(1).jpg" target="_blank" rel="noreferrer">구장 사진: Christophe95, CC BY-SA 4.0</a></p>
      </footer>
    </div>
  );
      ...
      <section className="hero-banner">
        ...
        <div className="hero-side">
          <section className="today-panel">
            <div className="today-panel-head"><span>— 오늘의 경기</span><strong>{new Intl.DateTimeFormat("ko-KR", { timeZone: KST_TIME_ZONE, year: "numeric", month: "long", day: "numeric" }).format(new Date())}</strong></div>
            {todayGames.length === 0 ? (
              <p className="today-empty">오늘 예정된 경기가 없습니다.</p>
            ) : (
              <div className="today-game-list">
                {todayGames.map((game) => (
                  <button key={game.gameId} className={selectedGame?.gameId === game.gameId ? "today-game active" : "today-game"} onClick={() => onSelectGame(game)}>
                    <span className={`status-pill ${game.bookingStatus.toLowerCase()}`}>{gameStatusMeta[game.bookingStatus].label}</span>
        ...
        <div className="calendar-shell">
          <div className="calendar-toolbar">
            ...
            <div className="schedule-filters">
              <label><span>구단별</span><select value={teamFilter} onChange={(event) => setTeamFilter(event.target.value)}><option value="ALL">전체 구단</option>{teams.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
              <label><span>구장별</span><select value={stadiumFilter} onChange={(event) => setStadiumFilter(event.target.value)}><option value="ALL">전체 구장</option>{stadiums.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
              <label><span>상태</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="ALL">전체 상태</option><option value="SCHEDULED">예매 예정</option><option value="OPEN">예매중</option><option value="CLOSED">예매 종료</option><option value="CANCELLED">경기 취소</option></select></label>
            </div>
          </div>
          ...
          <div className="calendar-grid">
            {calendarCells.map((day, index) => {
              ...
              return (
                <button className={`${selectedDate === dateKey ? "calendar-day selected" : "calendar-day"}${isToday ? " today" : ""}`} key={dateKey} onClick={() => selectCalendarDay(day)}>
                  <span>{day}</span>
                  {dayGames.length > 0 && <strong>{dayGames.length}경기</strong>}
                  <div>{dayGames.map((game) => <i className={game.bookingStatus.toLowerCase()} key={game.gameId} />)}</div>
                </button>
              );
            })}
          </div>
          <div className="calendar-legend"><span><i className="scheduled" />예매 예정</span><span><i className="open" />예매중</span><span><i className="closed" />예매 종료</span><span><i className="cancelled" />경기 취소</span></div>
        </div>

        <div className="list-heading"><div><strong>{selectedDate ? `${selectedDate.replace(/-/g, ".")} 경기` : "전체 경기"}</strong><span>{filteredGames.length}개 일정</span></div>{selectedDate && <button className="text-button" onClick={() => setSelectedDate(null)}>날짜 선택 해제</button>}</div>
        {(gamesResult?.data.length ?? 0) === 0 ? (
          ...
          <div className="game-list">
            {filteredGames.map((game) => {
              ...
              return (
                <article className={`${selected ? "game-card selected" : "game-card"} ${game.bookingStatus.toLowerCase()}`} key={game.gameId}>
                  <button className="game-card-main" onClick={() => onSelectGame(game)}>
                    <div className="game-date"><span>{date.month}</span><strong>{date.day}</strong><small>{date.weekday}</small></div>
                    <div className="match-info">
                      <span className={`status-pill ${game.bookingStatus.toLowerCase()}`}>{completed ? "예매 완료" : meta.label}</span>
                      <div className="match-teams"><strong>{game.homeTeam.name}</strong><span>VS</span><strong>{game.awayTeam.name}</strong></div>
                      <p>{game.title}</p><small>{formatGameDate(game.gameAt)} · {game.stadium.name} · {meta.description}</small>
                    </div>
                  </button>
                  <button className="card-book-button" onClick={() => { void onSelectGame(game); }} disabled={game.bookingStatus !== "OPEN" || completed}>
...
function QueueScreen({ game, queue, progress, busy, onRefresh, onCancel, onContinue }: {
  ...
  return (
    <section className="queue-page">
      <div className="compact-game-header"><span>{formatShortDate(game.gameAt).day}</span><div><strong>{game.homeTeam.name} <em>VS</em> {game.awayTeam.name}</strong><small>{formatGameDate(game.gameAt)} · {game.stadium.name}</small></div></div>
      <div className={admitted ? "queue-card admitted" : "queue-card"}>
        ...
        <div className="queue-meta">
          ...
          <div><span>예상 대기시간</span><strong>{queue?.estimatedWaitSeconds == null ? "계산 중" : `${queue.estimatedWaitSeconds}초`}</strong></div>
        </div>

        <div className="queue-actions">
          <button className="outline-button" onClick={onRefresh} disabled={busy}>상태 확인</button>
          <button className="primary-button" onClick={onContinue} disabled={!admitted || busy}>좌석 선택으로 이동 →</button>
        </div>
        {!admitted && <button className="text-button danger" onClick={onCancel} disabled={busy}>예매 취소하고 돌아가기</button>}
      </div>
      <div className="info-strip"><span>i</span><p><strong>대기 중에는 페이지를 닫지 마세요.</strong> 실시간 연결이 종료되더라도 상태 확인 버튼으로 현재 순번을 다시 조회할 수 있습니다.</p></div>
    </section>
  );
}
...
function SeatScreen({
  ...
  return (
    <section className="seat-page">
      <div className="compact-game-header"><span>{formatShortDate(game.gameAt).day}</span><div><strong>{game.homeTeam.name} <em>VS</em> {game.awayTeam.name}</strong><small>{formatGameDate(game.gameAt)} · {game.stadium.name}</small></div><div className="seat-legend"><span className="available" />선택 가능<span className="picked" />선택 좌석<span className="unavailable" />선택 불가</div></div>

      <div className="seat-layout">
        <div className="zone-panel">
          <div className="panel-title"><span>01</span><div><strong>구역 선택</strong><small>원하는 구역을 먼저 선택하세요.</small></div></div>
          <div className="stadium-mini"><img src={STADIUM_IMAGE_URL} alt="잠실야구장 경기 전경" /><span>JAMSIL STADIUM</span></div>
          <div className="zone-list">
            {(zonesResult?.data ?? []).map((zone) => (
              <button className={selectedZoneId === zone.zoneId ? "active" : ""} key={zone.zoneId} onClick={() => onSelectZone(zone.zoneId)} disabled={selectionLocked}>
                <div><strong>{zone.zoneName}</strong><small>{zone.grade === "INFIELD" ? "내야" : "외야"} · {formatPrice(zone.basePrice)}</small></div>
        ...
        <aside className="selection-panel">
          <div className="panel-title"><span>03</span><div><strong>선택 확인</strong><small>최대 2석까지 선택할 수 있습니다.</small></div></div>
          {timerTarget ? (
          ...
          <div className="selected-seat-list">
            {selectedSeats.length === 0 ? <p>좌석을 선택하면 이곳에 표시됩니다.</p> : selectedSeats.map((seat) => (
              <div key={seat.gameSeatId}><span><strong>{seat.zoneName}</strong><small>{seat.seatRow}열 {seat.seatNumber}번</small></span><strong>{formatPrice(seat.price)}</strong></div>
            ))}
          </div>
          <div className="total-line"><span>총 결제 예정 금액</span><strong>{formatPrice(totalAmount)}</strong></div>
          {!reservation ? (
            <button className="primary-button full" onClick={onReserve} disabled={selectedSeatIds.length === 0 || selectionLocked}>{timerExpired ? "좌석 선택 시간 만료" : `${selectedSeatIds.length}석 선점하기 →`}</button>
          ) : (
            <>
              <button className="primary-button full" onClick={onContinue} disabled={timerExpired || busy}>{order ? "주문 정보로 돌아가기 →" : "주문 정보 입력 →"}</button>
              {!order && <button className="outline-button full" onClick={onCancelReservation} disabled={busy}>선점 해제</button>}
            </>
          )}
          <small className="selection-help">{order ? "주문이 생성된 좌석은 변경할 수 없으며, 결제 기한까지 동일하게 유지됩니다." : "입장 후 5분 안에 좌석을 선점해야 하며, 선점 후에는 예약 만료시간이 적용됩니다."}</small>
        </aside>
      </div>
    </section>
  );
...
function CheckoutScreen({ game, seats, reservation, order, paymentPrepared, busy, onCreateOrder, onRefreshOrder, onCancelOrder, onPayment, onBack }: {
  game: GameSummary;
  seats: GameSeat[];
  reservation: ReservationResponse | null;
  order: OrderResponse | null;
  ...
  return (
    <section className="checkout-page">
      <div className="page-title"><button className="step-back-button" onClick={onBack} disabled={busy}>← 좌석 선택으로</button><span className="eyebrow">CHECKOUT</span><h1>예매 정보 확인</h1><p>선택한 경기와 좌석을 확인한 뒤 주문을 생성해주세요.</p></div>
      <div className="checkout-layout">
        <div className="checkout-details">
          <article className="detail-card game-summary"><img src={STADIUM_IMAGE_URL} alt="잠실야구장 경기 전경" /><div><span>경기 정보</span><h2>{game.homeTeam.name} <em>VS</em> {game.awayTeam.name}</h2><p>{formatGameDate(game.gameAt)}</p><small>{game.stadium.name}</small></div></article>
          <article className="detail-card"><div className="detail-card-head"><h2>선택 좌석</h2><span>{seats.length}석</span></div>{seats.map((seat) => <div className="checkout-seat" key={seat.gameSeatId}><span><strong>{seat.zoneName}</strong><small>{seat.seatRow}열 {seat.seatNumber}번</small></span><strong>{formatPrice(seat.price)}</strong></div>)}</article>
          <article className="detail-card"><div className="detail-card-head"><h2>수령 방법</h2><span>MOBILE</span></div><div className="delivery-option active"><span>▣</span><div><strong>모바일 티켓</strong><small>결제 완료 후 내 티켓에서 확인</small></div><b>✓</b></div></article>
        </div>

        <aside className="payment-summary">
          <h2>결제 금액</h2>
          <dl><dt>좌석 금액</dt><dd>{formatPrice(amount)}</dd><dt>예매 수수료</dt><dd>0원</dd></dl>
          <div className="total-line"><span>최종 결제금액</span><strong>{formatPrice(amount)}</strong></div>
          {deadlineTarget && <div className={deadlineExpired ? "deadline-line expired" : "deadline-line"}><span>{order ? "결제 남은 시간" : "선점 남은 시간"}</span><Countdown target={deadlineTarget} onExpire={() => setDeadlineExpired(true)} /></div>}
          {deadlineExpired && <p className="deadline-warning">제한시간이 만료되어 더 이상 진행할 수 없습니다.</p>}
          {!order ? (
            <button className="primary-button full" onClick={onCreateOrder} disabled={!reservation || busy || deadlineExpired}>주문 생성하기 →</button>
          ) : (
            <>
              <div className={`order-status ${order.status.toLowerCase()}`}><span>주문번호</span><strong>{order.orderNo}</strong><small>{order.status}</small></div>
              <button className="primary-button full" onClick={onPayment} disabled={order.status !== "CREATED" || busy || deadlineExpired}>{deadlineExpired ? "결제 시간 만료" : paymentPrepared ? "결제 화면으로 이동 →" : `${formatPrice(order.totalAmount)} 결제 준비 →`}</button>
              <button className="outline-button full" onClick={onRefreshOrder} disabled={busy}>주문 상태 확인</button>
              <button className="text-button danger" onClick={onCancelOrder} disabled={order.status !== "CREATED" || busy}>주문 취소</button>
            </>
          )}
  ...
  return (
    <section className="payment-page">
      <div className={approved ? "payment-result approved" : "payment-result"}>
        {!approved && <button className="step-back-button" onClick={onBack} disabled={busy}>← 주문으로 돌아가기</button>}
        ...
        {!approved && deadlineExpired && <p className="deadline-warning centered">결제시간이 만료되었습니다. 주문 상태를 확인해주세요.</p>}
        <div className="payment-receipt">
          ...
          <div className="receipt-total"><span>총 결제금액</span><strong>{formatPrice(payment?.amount ?? order?.totalAmount ?? 0)}</strong></div>
        </div>
        {approved ? (
          <button className="primary-button" onClick={onTickets}>내 티켓 확인 →</button>
        ) : (
          <div className="payment-actions">
            <button className="primary-button" onClick={onOpenPayment} disabled={!payment || payment.status !== "READY" || busy || deadlineExpired}>{deadlineExpired ? "결제 시간 만료" : "Toss 결제창 열기 →"}</button>
            <button className="outline-button" onClick={onRefreshOrder} disabled={!order || busy}>주문 상태 확인</button>
          </div>
        )}
      </div>
    </section>
