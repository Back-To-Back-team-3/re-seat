...
import {cancelTicket, getTickets, retryCancelTicket} from "@/api/tickets";
...
export function useRetryCancelTicket() {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: (ticketId: number) => retryCancelTicket(ticketId),
        onSuccess: (result) => {
            queryClient.setQueryData<TicketSummary[]>(ticketKeys.list(), (old) => {
                if (!old) return old;
                return old.map((ticket) =>
