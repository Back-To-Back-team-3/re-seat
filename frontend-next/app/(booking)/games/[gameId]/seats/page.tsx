  ...
  return (
    <section className="mx-auto w-[min(1320px,100%)]">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_310px] items-start gap-[14px] max-[1024px]:grid-cols-1">
        <div className="col-start-1 row-start-1 grid grid-cols-[minmax(240px,0.7fr)_minmax(0,1.3fr)] overflow-hidden rounded-panel border border-border bg-surface max-[1024px]:col-start-auto max-[1024px]:row-start-auto max-[640px]:grid-cols-1">
          <div className="col-span-2 flex items-center gap-2.5 border-b border-border px-[18px] py-[17px] max-[640px]:col-span-1">
          ...
          <div className="relative m-[14px] min-h-[220px] overflow-hidden rounded-[9px] bg-surface-soft max-[640px]:h-[155px] max-[640px]:min-h-[155px]">
            <img
              alt="잠실야구장 경기 전경"
              className="size-full object-cover opacity-[0.82]"
              src={STADIUM_IMAGE_URL}
            />
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 font-mono text-[7px] tracking-[2px] text-muted-foreground">
              JAMSIL STADIUM
            </span>
          </div>

          <div className="grid max-h-[470px] grid-cols-2 content-start gap-1.5 overflow-y-auto px-[14px] pb-[14px] max-[900px]:max-h-[280px] max-[900px]:pt-[14px] max-[640px]:grid-cols-1">
            {zones.data?.map((zone) => (
              <button
