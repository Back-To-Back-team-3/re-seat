...
export function CheckoutScreen(props: CheckoutScreenProps) {
  ...
  return (
    <section className="mx-auto w-[min(1120px,100%)]">
      ...
      <div className="grid grid-cols-[minmax(0,1.45fr)_minmax(300px,0.75fr)] items-start gap-4 max-[900px]:grid-cols-1">
        <OrderSummary game={game} seats={seats} />

        <aside className="sticky top-[100px] overflow-hidden rounded-panel border border-border bg-surface p-5 max-[900px]:static">
          ...
          {!order ? (
            ...
            <>
              <div className="mb-3 grid grid-cols-[1fr_auto] gap-x-3 gap-y-1 rounded-[7px] bg-surface-soft p-3">
                <span className="text-xs text-muted-foreground">주문번호</span>
                <strong className="col-start-1 row-start-2 overflow-hidden font-mono text-xs text-ellipsis">
                  {order.orderNo}
                </strong>
                <small className="col-start-2 row-span-2 row-start-1 self-center text-xs font-black text-brand">
                  {order.status}
                </small>
              </div>
              <button
