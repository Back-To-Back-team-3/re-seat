...
public class OrderResponse {
    ...
    @Schema(
        description = "선점 만료 시간",
        example = "2026-07-11 14:30:00"
    )
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
     ...
     * @return 주문 응답 DTO
     */
    public static OrderResponse from(Order order, List<OrderItem> orderItems, LocalDateTime holdExpiresAt) {
        return OrderResponse
            ...
            .orderItems(orderItems.stream().map(OrderItemResponse::from).toList())
            .build();
    }
    ...
    public static class OrderItemResponse {
        ...
        @Schema(
            description = "경기 좌석 ID",
            example = "5001"
        )
        private final Long gameSeatId;

        @Schema(
            description = "주문 확정 가격",
            example = "18000"
        )
