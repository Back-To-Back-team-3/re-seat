...
public interface PaymentControllerDocs {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "INVALID_REQUEST / IDEMPOTENCY_KEY_REQUIRED", content = @Content),
        ...
        @ApiResponse(responseCode = "409", description = "IDEMPOTENCY_KEY_CONFLICT / INVALID_ORDER_STATUS / LOCK_FAILED", content = @Content),
    ...
    ResponseEntity<com.backtoback.reseat.global.common.ApiResponse<PaymentCreateResponse>> requestPayment(
        ...
        @Parameter(description = "결제 요청을 식별하는 멱등키", example = "8f14e45f-ea5e-4a2f-b3d2-09bcdb51f321", required = true)
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "INVALID_REQUEST / IDEMPOTENCY_KEY_REQUIRED / PAYMENT_CALLBACK_MISMATCH", content = @Content),
        @ApiResponse(responseCode = "401", description = "인증 실패", content = @Content),
        @ApiResponse(responseCode = "403", description = "PAYMENT_ACCESS_DENIED", content = @Content),
        @ApiResponse(responseCode = "404", description = "PAYMENT_NOT_FOUND", content = @Content),
        @ApiResponse(responseCode = "409", description = "IDEMPOTENCY_KEY_UNAVAILABLE / INVALID_ORDER_STATUS", content = @Content),
        @ApiResponse(responseCode = "410", description = "ORDER_EXPIRED", content = @Content)
    })
    ...
    @Operation(summary = "결제 단건 조회", description = "결제 ID로 본인의 결제 상세 상태와 처리 결과를 조회합니다.", security = @SecurityRequirement(name = "JWT Bearer Token"))
    @ApiResponses({
