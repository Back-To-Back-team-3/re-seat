...
public interface PaymentControllerDocs {
    ...
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", description = "결제 요청 처리 성공"),
        @ApiResponse(
            responseCode = "400", description = "INVALID_REQUEST / IDEMPOTENCY_KEY_REQUIRED",
            content = @Content),
        @ApiResponse(
            responseCode = "401", description = "인증 실패", content = @Content),
        @ApiResponse(
            responseCode = "403", description = "PAYMENT_ACCESS_DENIED", content = @Content),
        @ApiResponse(
            responseCode = "404", description = "ORDER_NOT_FOUND", content = @Content),
        @ApiResponse(
            responseCode = "409",
            description = "IDEMPOTENCY_KEY_CONFLICT / INVALID_ORDER_STATUS / LOCK_FAILED",
            content = @Content),
        @ApiResponse(
            responseCode = "410", description = "ORDER_EXPIRED", content = @Content)
    })
    ...
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", description = "결제 승인 처리 성공 또는 기존 종결 결과 반환"),
        @ApiResponse(
            responseCode = "400",
        ...
        @ApiResponse(
            responseCode = "403", description = "PAYMENT_ACCESS_DENIED", content = @Content),
        @ApiResponse(
            responseCode = "404", description = "PAYMENT_NOT_FOUND", content = @Content),
        @ApiResponse(
            responseCode = "409",
    ...
    ResponseEntity<com.backtoback.reseat.global.common.ApiResponse<PaymentActionResponse>> completePayment(
        @Parameter(hidden = true) CustomUserDetails userDetails,
        @Parameter(description = "결제 ID", example = "1001", required = true) Long paymentId,
        @Parameter(
            description = "현재 결제 시도의 활성 멱등키",
            example = "8f14e45f-ea5e-4a2f-b3d2-09bcdb51f321",
            required = true
        )
        String idempotencyKey,
    ...
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", description = "결제 실패 처리 성공 또는 기존 종결 결과 반환"),
        @ApiResponse(
            responseCode = "400",
            description = "INVALID_REQUEST / IDEMPOTENCY_KEY_REQUIRED / PAYMENT_CALLBACK_MISMATCH",
            content = @Content),
        ...
        @ApiResponse(
            responseCode = "404", description = "PAYMENT_NOT_FOUND", content = @Content),
        @ApiResponse(
            responseCode = "409", description = "IDEMPOTENCY_KEY_UNAVAILABLE", content = @Content)
    })
    ...
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", description = "결제 취소 처리 성공 또는 기존 취소 결과 반환"),
        @ApiResponse(
            responseCode = "400", description = "INVALID_REQUEST", content = @Content),
        @ApiResponse(
            responseCode = "401", description = "인증 실패", content = @Content),
        @ApiResponse(
            responseCode = "403", description = "PAYMENT_ACCESS_DENIED", content = @Content),
        @ApiResponse(
            responseCode = "404", description = "PAYMENT_NOT_FOUND", content = @Content),
        @ApiResponse(
            responseCode = "409", description = "PAYMENT_CANCEL_NOT_ALLOWED", content = @Content),
        @ApiResponse(
            responseCode = "502", description = "PAYMENT_CANCEL_FAILED", content = @Content)
    })
    ...
    @ApiResponses({
        @ApiResponse(
            responseCode = "200", description = "결제 조회 성공"),
        @ApiResponse(
            responseCode = "401", description = "인증 실패", content = @Content),
        @ApiResponse(
            responseCode = "403", description = "PAYMENT_ACCESS_DENIED", content = @Content),
