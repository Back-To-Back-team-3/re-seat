...
@Tag(
    name = "Queue",
    description = "대기열 진입 · 상태 조회 · SSE · 취소 API"
)
public interface QueueControllerDocs {

    @Operation(
        summary = "대기열 진입 요청",
        description = "경기 대기열 진입 요청을 비동기로 접수하며, 처리 완료 전에는 대기 상태가 조회되지 않을 수 있습니다.",
    ...
    @ApiResponses(
        {
            ...
            @ApiResponse(
                responseCode = "404",
                description = "GAME_NOT_FOUND / USER_NOT_FOUND",
                content = @Content
            ),
            @ApiResponse(
                responseCode = "503",
                description = "QUEUE_EVENT_PUBLISH_FAILED",
                content = @Content
            )
        }
    )
    ...
    @Operation(
        summary = "내 대기 상태 조회",
        description = "현재 대기 순번, 예상 대기시간과 입장 허용 여부를 조회합니다.",
        security = @SecurityRequirement(name = "JWT Bearer Token")
    )
    @ApiResponses(
        {
    ...
    @Operation(
        summary = "대기 순번 실시간 스트림 (SSE)",
        description = "SSE로 대기 상태와 입장 허용 정보를 전송하며, 대기중 마지막 연결이 종료되면 60초 동안 재연결을 허용합니다.",
        security = @SecurityRequirement(name = "JWT Bearer Token")
    )
    @ApiResponses(
        {
    ...
    @Operation(
        summary = "대기열 취소",
        description = "현재 대기열 진입을 취소하고 활성 Queue-Token이 있으면 함께 무효화합니다.",
        security = @SecurityRequirement(name = "JWT Bearer Token")
    )
    @ApiResponses(
        {
            ...
            @ApiResponse(
                responseCode = "404",
                description = "QUEUE_ENTRY_NOT_FOUND / USER_NOT_FOUND",
                content = @Content
            ),
            @ApiResponse(
                responseCode = "409",
                description = "QUEUE_INVALID_STATUS",
                content = @Content
            )
        }
    )
