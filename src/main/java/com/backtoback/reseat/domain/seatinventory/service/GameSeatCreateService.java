...
public class GameSeatCreateService {
	...
	public GameSeatOpenResponse openInventory(Long gameId) {
		...
		if (seats.isEmpty()) {
			log.error("좌석 기준 데이터가 없는 구장에 재고 오픈이 시도되었습니다. gameId={}, stadiumId={}",
				gameId, stadiumId);
			throw new IllegalStateException("구장에 등록된 활성 좌석이 없습니다. stadiumId=" + stadiumId);
		}

		// 4. 좌석별 가격 산정 후 재고 생성
		List<GameSeat> gameSeats = seats.stream()
			.map(seat -> createGameSeat(game, seat))
			.toList();
		...
		log.info("좌석 재고 오픈 완료. gameId={}, stadiumId={}, createdCount={}, elapsedMs={}",
			gameId, stadiumId, savedGameSeats.size(), elapsedMillis);
