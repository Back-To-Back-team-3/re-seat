...
public record ReservationResponse(
    ...
    @Schema(description = "선점된 좌석 목록 (좌석별 상태·가격)") List<SeatHoldInfo> gameSeats,

    /** 선점 만료 시각 (now + 5분). 만료 스케줄러는 C-3에서 구현. */
    @Schema(
        description = "선점 만료 시각",
        example = "2026-07-11T18:37:00"
    ...
    public static ReservationResponse from(Reservation reservation) {
        List<SeatHoldInfo> seats = reservation.getReservationSeats().stream().map(SeatHoldInfo::from).toList();
    ...
    public record SeatHoldInfo(
        ...
        @Schema(
            description = "경기 좌석 재고 ID (game_seats.id)",
            example = "5001"
        ) Long gameSeatId,
        ...
        @Schema(
            description = "선점 당시 가격 (price 스냅샷)",
            example = "18000"
        ) int price
    ) {
