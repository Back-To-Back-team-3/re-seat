...
class PaymentOrderPolicyTest {
	...
	class GetOwnedOrder {
		...
		void rejectsMissingOrder() {
			...
			assertThatThrownBy(() -> paymentOrderPolicy.getOwnedOrder(USER_ID, ORDER_ID)).isInstanceOf(
	...
	class EnsurePayable {
		...
		void expiresExistingPaymentAndOrderAfterDeadline() {
			...
			assertThatThrownBy(() -> paymentOrderPolicy.ensurePayable(payment, order)).isInstanceOf(
		...
		void expiresOrderWithoutPaymentAfterDeadline() {
			...
			assertThatThrownBy(() -> paymentOrderPolicy.ensurePayable(
				null,
				order
		...
		void rejectsNonCreatedOrder(OrderStatus status) {
			...
			assertThatThrownBy(() -> paymentOrderPolicy.ensurePayable(null, order)).isInstanceOf(
				PaymentOrderNotPayableException.class);
