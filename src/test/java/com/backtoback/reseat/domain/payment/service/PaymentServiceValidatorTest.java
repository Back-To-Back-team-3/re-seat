...
class PaymentServiceValidatorTest {
	...
	class ValidateIdempotencyKey {
		...
		void rejectsMissingKey() {
			assertThatThrownBy(() -> validator.validateIdempotencyKey(null)).isInstanceOf(
				IdempotencyKeyRequiredException.class);
			assertThatThrownBy(() -> validator.validateIdempotencyKey(" ")).isInstanceOf(IdempotencyKeyRequiredException.class);
		}
	}
	...
	class ValidateIdempotencyRequest {
		...
		void rejectsDifferentOrder() {
			...
			assertThatThrownBy(() -> validator.validateIdempotencyRequest(payment, 999L)).isInstanceOf(
	...
	class ValidateActiveIdempotencyKey {
		...
		void acceptsActiveKey() {
			...
			assertThatCode(() -> validator.validateActiveIdempotencyKey(
				payment,
				IDEMPOTENCY_KEY
		...
		void rejectsInactiveKey() {
			...
			assertThatThrownBy(() -> validator.validateActiveIdempotencyKey(payment, "inactive-key")).isInstanceOf(
				IdempotencyKeyUnavailableException.class);
		}
	}
	...
	class ValidateOwner {
		...
		void rejectsOtherUser() {
			...
			assertThatThrownBy(() -> validator.validateOwner(
				payment,
				999L
	...
	class ValidateConfirmable {
		...
		void acceptsMatchingReadyPayment() {
			...
			assertThatCode(() -> validator.validateConfirmable(
				payment,
				PG_ORDER_ID,
				AMOUNT
			)).doesNotThrowAnyException();
		...
		void rejectsFinalizedPayment(PaymentStatus status) {
			...
			assertThatThrownBy(() -> validator.validateConfirmable(payment, PG_ORDER_ID, AMOUNT)).isInstanceOf(
				PaymentAlreadyFinalizedException.class);
		}
	}
	...
	class ValidateFailable {
		...
		void rejectsFinalizedPayment(PaymentStatus status) {
			...
			assertThatThrownBy(() -> validator.validateFailable(payment)).isInstanceOf(PaymentAlreadyFinalizedException.class);
		}
	}
	...
	class ValidateCancelable {
		...
		void rejectsUnapprovedPayment(PaymentStatus status) {
			...
			assertThatThrownBy(() -> validator.validateCancelable(payment)).isInstanceOf(
				PaymentCancelNotAllowedException.class);
		}
	...
	class ValidatePgOrderId {
		...
		void rejectsDifferentPgOrderId() {
			...
			assertThatThrownBy(() -> validator.validatePgOrderId(payment, "different-order")).isInstanceOf(
				PaymentCallbackMismatchException.class);
		}
	}
	...
	class ValidateAmount {
		...
		void rejectsDifferentAmount() {
			...
			assertThatThrownBy(() -> validator.validateAmount(payment, AMOUNT + 1)).isInstanceOf(
				PaymentCallbackMismatchException.class);
		}
	}
}
