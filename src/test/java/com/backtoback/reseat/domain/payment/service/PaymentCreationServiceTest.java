...
class PaymentCreationServiceTest {
	...
	class RequestPayment {
		...
		void returnsExistingReadyPaymentForSameKey() {
			...
			when(paymentRepository.findByIdempotencyKeyWithPessimisticWriteLock(IDEMPOTENCY_KEY)).thenReturn(Optional.of(
				payment));
		...
		void rejectsSameKeyForDifferentOrder() {
			...
			assertThatThrownBy(() -> paymentCreationService.requestPayment(
				USER_ID,
				IDEMPOTENCY_KEY,
				request
		...
		void replacesIdempotencyKeyOfExistingReadyPayment() {
			...
			Order order = order();
			Payment payment = payment(order, PaymentStatus.READY, IDEMPOTENCY_KEY);
			when(paymentRepository.findByIdempotencyKeyWithPessimisticWriteLock(NEW_IDEMPOTENCY_KEY)).thenReturn(
				Optional.empty());
			...
			PaymentCreateResponse response = paymentCreationService.requestPayment(
				USER_ID,
				NEW_IDEMPOTENCY_KEY,
				request
			);
		...
		void createsReadyPaymentWhenPaymentDoesNotExist() {
			...
			when(paymentRepository.findByIdempotencyKeyWithPessimisticWriteLock(IDEMPOTENCY_KEY)).thenReturn(Optional.empty());
			when(paymentOrderPolicy.getOwnedOrder(USER_ID, ORDER_ID)).thenReturn(order);
