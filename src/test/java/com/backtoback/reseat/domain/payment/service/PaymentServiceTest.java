...
class PaymentServiceTest {
	...
	class RequestPayment {
		...
		void rejectsWhenLockCannotBeAcquired() throws InterruptedException {
			...
			assertThatThrownBy(() -> paymentService.requestPayment(USER_ID, IDEMPOTENCY_KEY, request)).isInstanceOf(
				PaymentLockFailedException.class);
		...
		void unlocksWhenCreationFails() throws InterruptedException {
			...
			assertThatThrownBy(() -> paymentService.requestPayment(USER_ID, IDEMPOTENCY_KEY, request)).isSameAs(
				exception);
	...
	class CompletePayment {
		...
		void registersRecoveryTaskForUnknownConfirmStatus() {
			...
			when(tossPaymentClient.confirm(
				...
				"승인",
				new RuntimeException("Toss 응답 없음")
			));
	...
	class CancelPayment {
		...
		void rejectsUnapprovedPayment(PaymentStatus status) {
			...
			assertThatThrownBy(() -> paymentService.cancelPayment(USER_ID, PAYMENT_ID, request)).isInstanceOf(
		...
		void keepsApprovedPaymentWhenCancelStatusIsUnknown() {
			...
			assertThatThrownBy(() -> paymentService.cancelPayment(USER_ID, PAYMENT_ID, request)).isInstanceOf(
				PaymentCancelFailedException.class);
	...
	class GetPayment {
		...
		void rejectsMissingPayment() {
			...
			assertThatThrownBy(() -> paymentService.getPayment(USER_ID, PAYMENT_ID)).isInstanceOf(
