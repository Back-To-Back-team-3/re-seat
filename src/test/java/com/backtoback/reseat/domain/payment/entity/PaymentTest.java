...
class PaymentTest {
	...
	class AssignPgPaymentKey {
		...
		void rejectsDifferentKey() {
			...
			assertThatThrownBy(() -> payment.assignPgPaymentKey("different-payment-key")).isInstanceOf(
	...
	class ChangeIdempotencyKey {
		...
		void rejectsFinalizedPayment(PaymentStatus status) {
			...
			assertThatThrownBy(() -> payment.changeIdempotencyKey("new-idempotency-key")).isInstanceOf(
