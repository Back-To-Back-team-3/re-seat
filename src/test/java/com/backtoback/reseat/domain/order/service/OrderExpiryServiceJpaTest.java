...
public class OrderExpiryServiceJpaTest {
    ...
    private OrderExpiryFixture createOrderExpiryFixture(
        ...
        Order order
            = Order.of("ORD-EXPIRY-" + suffix, user, reservation, PRICE * gameSeatStatus.length, paymentDeadline);
        ...
        return new OrderExpiryFixture(order.getId(), reservation.getId(), List.copyOf(gameSeatIds));
    }
    ...
    void expireOrders_expiresOverdueOrderAndReleasesHold() {
        ...
        // given
        OrderExpiryFixture overdueCreatedOrderFixture
            = createOrderExpiryFixture(
        ...
        Order expiredOrder = entityManager.find(Order.class, overdueCreatedOrderFixture.orderId());
        assertThat(expiredOrder.getStatus()).isEqualTo(OrderStatus.EXPIRED);

        Reservation expiredReservation
            = entityManager.find(Reservation.class, overdueCreatedOrderFixture.reservationId());
        assertThat(expiredReservation.getStatus()).isEqualTo(ReservationStatus.EXPIRED);

        List<GameSeat> gameSeats
            = overdueCreatedOrderFixture
                .gameSeatIds()
                .stream()
                .map(gameSeatId -> entityManager.find(GameSeat.class, gameSeatId))
                .toList();
    ...
    void expireOrders_expiresOrderAtPaymentDeadline() {
        ...
        // given
        OrderExpiryFixture deadlineBoundaryCreatedOrderFixture
            = createOrderExpiryFixture(NOW, OrderStatus.CREATED, ReservationStatus.HOLDING, GameSeatStatus.HELD);
        ...
        assertThat(result.expiredOrders()).isEqualTo(1);
        assertThat(result.expiredReservations()).isEqualTo(1);
        assertThat(result.releasedSeats()).isEqualTo(1);
        assertThat(result.total()).isEqualTo(3);

        Order expiredOrder = entityManager.find(Order.class, deadlineBoundaryCreatedOrderFixture.orderId());
        assertThat(expiredOrder.getStatus()).isEqualTo(OrderStatus.EXPIRED);

        Reservation expiredReservation
            = entityManager.find(Reservation.class, deadlineBoundaryCreatedOrderFixture.reservationId());
        assertThat(expiredReservation.getStatus()).isEqualTo(ReservationStatus.EXPIRED);

        List<GameSeat> gameSeats
            = deadlineBoundaryCreatedOrderFixture
                .gameSeatIds()
                .stream()
                .map(gameSeatId -> entityManager.find(GameSeat.class, gameSeatId))
                .toList();
    ...
    void expireOrders_keepsIneligibleOrders() {
        ...
        // given
        OrderExpiryFixture futureCreatedOrderFixture
            = createOrderExpiryFixture(
        ...
        OrderExpiryFixture overdueOrderFixture
            = createOrderExpiryFixture(
                EXPIRED_PAYMENT_DEADLINE,
                OrderStatus.PAID,
        ...
        // then
        assertThat(result.expiredOrders()).isEqualTo(0);
        assertThat(result.expiredReservations()).isEqualTo(0);
        assertThat(result.releasedSeats()).isEqualTo(0);
        assertThat(result.total()).isEqualTo(0);

        Order fetureOrder = entityManager.find(Order.class, futureCreatedOrderFixture.orderId());
        Order paidOrder = entityManager.find(Order.class, overdueOrderFixture.orderId());
        assertThat(fetureOrder.getStatus()).isEqualTo(OrderStatus.CREATED);
        assertThat(paidOrder.getStatus()).isEqualTo(OrderStatus.PAID);

        Reservation holdingReservation
            = entityManager.find(Reservation.class, futureCreatedOrderFixture.reservationId());
        Reservation confirmedReservation = entityManager.find(Reservation.class, overdueOrderFixture.reservationId());
        assertThat(holdingReservation.getStatus()).isEqualTo(ReservationStatus.HOLDING);
        assertThat(confirmedReservation.getStatus()).isEqualTo(ReservationStatus.CONFIRMED);

        GameSeat heldGameSeat = entityManager.find(GameSeat.class, futureCreatedOrderFixture.gameSeatIds().get(0));
        GameSeat soldGameSeat = entityManager.find(GameSeat.class, overdueOrderFixture.gameSeatIds().get(0));
        assertThat(heldGameSeat.getStatus()).isEqualTo(GameSeatStatus.HELD);
        assertThat(soldGameSeat.getStatus()).isEqualTo(GameSeatStatus.SOLD);
        assertThat(heldGameSeat.getHoldExpiresAt()).isEqualTo(HOLD_EXPIRES_AT);
        assertThat(soldGameSeat.getHoldExpiresAt()).isNull();
    }
    ...
    void expireOrders_keepsReservationAndSeatWithNonTargetStatus() {
        ...
        // given
        OrderExpiryFixture overdueOrderWithNonTargetResourcesFixture
            = createOrderExpiryFixture(
                EXPIRED_PAYMENT_DEADLINE,
                OrderStatus.CREATED,
        ...
        Order expiredOrder = entityManager.find(Order.class, overdueOrderWithNonTargetResourcesFixture.orderId());
        assertThat(expiredOrder.getStatus()).isEqualTo(OrderStatus.EXPIRED);

        Reservation canceledReservation
            = entityManager.find(Reservation.class, overdueOrderWithNonTargetResourcesFixture.reservationId());
        assertThat(canceledReservation.getStatus()).isEqualTo(ReservationStatus.CANCELED);

        GameSeat availableGameSeat
            = entityManager.find(GameSeat.class, overdueOrderWithNonTargetResourcesFixture.gameSeatIds().get(0));
        assertThat(availableGameSeat.getStatus()).isEqualTo(GameSeatStatus.AVAILABLE);
        assertThat(availableGameSeat.getHoldExpiresAt()).isNull();
    }
    ...
    void expireOrders_doesNotProcessExpiredOrderAgain() {
        ...
        // then
        assertThat(firstExpiryResult.expiredOrders()).isEqualTo(1);
        assertThat(firstExpiryResult.expiredReservations()).isEqualTo(1);
        assertThat(firstExpiryResult.releasedSeats()).isEqualTo(1);
        assertThat(firstExpiryResult.total()).isEqualTo(3);

        assertThat(secondExpiryResult.expiredOrders()).isEqualTo(0);
        assertThat(secondExpiryResult.expiredReservations()).isEqualTo(0);
        assertThat(secondExpiryResult.releasedSeats()).isEqualTo(0);
        assertThat(secondExpiryResult.total()).isEqualTo(0);
        ...
        GameSeat availableGameSeat
            = entityManager.find(GameSeat.class, overdueCreatedOrderFixture.gameSeatIds().get(0));
        assertThat(availableGameSeat.getStatus()).isEqualTo(GameSeatStatus.AVAILABLE);
        assertThat(availableGameSeat.getHoldExpiresAt()).isNull();
    }

    private void changeReservationStatus(Reservation reservation, ReservationStatus reservationStatus) {
    ...
    private record OrderExpiryFixture(Long orderId, Long reservationId, List<Long> gameSeatIds) {
    }
}
