# Reservation 성능 테스트의 준비·초기화·정리 스크립트에서 공통으로 사용하는 함수를 정의합니다.
# 각 도메인 스크립트가 저장소 경로($script:RepoRoot, $script:ComposeFile)와
# $script:ExpectedTestType(예: 'reservation-performance')을 설정한 뒤 이 파일을 불러옵니다.
# testType 하드코딩을 변수로 분리해 도메인 무관하게 재사용할 수 있도록 이 폴더에 자체 보관합니다.
...
# 호출하는 도메인 스크립트가 dot-source 전에 $script:ExpectedTestType을 지정해야 한다.
function Resolve-ManifestPath {
    ...
    if ([string]::IsNullOrWhiteSpace($script:ExpectedTestType)) { throw '$script:ExpectedTestType이 설정되지 않았습니다. 도메인 스크립트에서 dot-source 전에 지정해주세요.' }
    $runsPath = Join-Path $script:RepoRoot 'build/k6/runs'
    if (-not (Test-Path -LiteralPath $runsPath)) { throw "성능 테스트 manifest를 찾지 못했습니다. -ManifestPath를 지정해주세요. (testType=$script:ExpectedTestType)" }
    $latest = Get-ChildItem -LiteralPath $runsPath -Filter 'manifest.json' -Recurse -File |
        Sort-Object LastWriteTime -Descending |
        Where-Object { (Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json).testType -eq $script:ExpectedTestType } |
        Select-Object -First 1
    if (-not $latest) { throw "성능 테스트 manifest를 찾지 못했습니다. -ManifestPath를 지정해주세요. (testType=$script:ExpectedTestType)" }
    $latest.FullName
}